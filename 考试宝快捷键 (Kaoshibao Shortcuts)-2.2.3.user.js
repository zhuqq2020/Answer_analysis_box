...
(function () {
    ...
    function unlockVIP() {
        ...
        selectorsToRemove.forEach(sel => {
            document.querySelectorAll(sel).forEach(el => {
                // 确保不删除导航栏和次导航栏相关元素
                if (!el.closest('.el-header') && !el.closest('header') && !el.closest('.breadcrumb')) {
                    el.remove();
                }
            });
        });

        // 移除"开通VIP查看完整解析"相关元素（不包含导航栏和次导航栏）
        ...
        for (let i = 0; i < vipNodes.snapshotLength; i++) {
            ...
            if (target && !target.classList.contains('app-main') &&
                ...
                !target.closest('.el-header') && !target.closest('header') && !target.closest('.breadcrumb')) {
                target.remove();
            } else if (!node.closest('.el-header') && !node.closest('header') && !node.closest('.breadcrumb')) {
                node.remove();
            }
        }
        ...
        for (let i = 0; i < deepNodes.snapshotLength; i++) {
            ...
            if (wrapper && !wrapper.classList.contains('app-main') &&
                !wrapper.closest('.el-header') && !wrapper.closest('header') && !wrapper.closest('.breadcrumb')) {
                wrapper.remove();
            }
        }

        // 清理绝对定位的 VIP 图标（不包含导航栏和次导航栏）
        document.querySelectorAll('i, img, svg').forEach(icon => {
            ...
            if (parentText.includes('VIP') || parentText.includes('解析')) {
                ...
                if ((style.position === 'absolute' || style.position === 'fixed') &&
                    !icon.closest('.el-header') && !icon.closest('header') && !icon.closest('.breadcrumb')) {
                    icon.remove();
                }
            }
        });
    ...
    function createSettingsUI() {
        ...
        // 构建面板 HTML
        let html = `<div class="ksb-title">⚡ 考试宝助手 v2.2.3 设置</div>`;

...
(function() {
    ...
    function updateStyles() {
        ...
        if (userConfig.shadowEnabled) {
            shadowValue = `${userConfig.shadowX}px ${userConfig.shadowY}px ${userConfig.shadowBlur}px ${userConfig.shadowSpread}px ${userConfig.shadowColor}`;
        }
        ...
        styleElement.textContent = `
            ...
            /* 面板拖动效果 */
            .aa-settings-header.dragging {
                opacity: 0.9 !important;
            }

            /* 滚动条美化 */
            .aa-settings-content::-webkit-scrollbar {
                width: 8px !important;
            }

            .aa-settings-content::-webkit-scrollbar-track {
                background: #f1f1f1 !important;
                border-radius: 4px !important;
            }

            .aa-settings-content::-webkit-scrollbar-thumb {
                background: ${userConfig.borderColor} !important;
                border-radius: 4px !important;
            }

            .aa-settings-content::-webkit-scrollbar-thumb:hover {
                background: #3a5a9f !important;
            }
        `;
    }
                ...
                <div class="aa-settings-group">
                    ...
                    <div class="aa-setting-item">
                        ...
                        <input type="number" class="aa-setting-input" id="aa-fontSize"
                               value="${userConfig.fontSize}" min="10" max="20" title="10-20px">
                    </div>
                    <div class="aa-setting-item">
                        <span class="aa-setting-label">行高</span>
                        <input type="number" class="aa-setting-input" id="aa-lineHeight"
                               value="${userConfig.lineHeight}" step="0.1" min="1.2" max="2.0" title="1.2-2.0">
                    </div>
                    <div class="aa-setting-item">
                        <span class="aa-setting-label">字体样式</span>
                        <select class="aa-setting-input" id="aa-fontFamily" style="width: 110px;">
                ...
                <div class="aa-settings-group">
                    ...
                    <div class="aa-setting-item">
                        ...
                        <input type="color" class="aa-setting-color" id="aa-backgroundColor"
                               value="${userConfig.backgroundColor}" title="点击选择颜色">
                    </div>
                    <div class="aa-setting-item">
                        <span class="aa-setting-label">边框颜色</span>
                        <input type="color" class="aa-setting-color" id="aa-borderColor"
                               value="${userConfig.borderColor}" title="点击选择颜色">
                    </div>
                    <div class="aa-setting-item">
                        <span class="aa-setting-label">文字颜色</span>
                        <input type="color" class="aa-setting-color" id="aa-textColor"
                               value="${userConfig.textColor}" title="点击选择颜色">
                    </div>
                </div>

                <div class="aa-settings-group">
                    <div class="aa-settings-title">阴影设置</div>
                    <div class="aa-setting-item">
                        <span class="aa-setting-label">启用阴影效果</span>
                        <div class="aa-setting-switch ${userConfig.shadowEnabled ? 'active' : ''}"
                             data-setting="shadowEnabled" title="点击切换"></div>
                    </div>
                    <div class="aa-setting-item">
                        <span class="aa-setting-label">阴影颜色</span>
                        <input type="color" class="aa-setting-color" id="aa-shadowColor"
                               value="${rgbToHex(userConfig.shadowColor)}" title="点击选择颜色">
                    </div>
                    <div class="aa-setting-item">
                        <span class="aa-setting-label">水平偏移</span>
                        <input type="number" class="aa-setting-input" id="aa-shadowX"
                               value="${userConfig.shadowX}" min="-10" max="10" title="-10到10">
                    </div>
                    <div class="aa-setting-item">
                        <span class="aa-setting-label">垂直偏移</span>
                        <input type="number" class="aa-setting-input" id="aa-shadowY"
                               value="${userConfig.shadowY}" min="0" max="20" title="0到20">
                    </div>
                    <div class="aa-setting-item">
                        <span class="aa-setting-label">模糊半径</span>
                        <input type="number" class="aa-setting-input" id="aa-shadowBlur"
                               value="${userConfig.shadowBlur}" min="0" max="30" title="0到30">
                    </div>
                </div>
    ...
    function bindSettingsEvents() {
        ...
        if (closeBtn) {
            ...
            const newCloseBtn = settingsPanel.querySelector('#aa-settings-close');
        ...
        // 开关切换
        const switches = settingsPanel.querySelectorAll('.aa-setting-switch');
        ...
        settingsPanel.querySelectorAll('.aa-setting-switch').forEach(switchEl => {
            switchEl.addEventListener('click', function(e) {
                ...
                if (settingName) {
                    ...
                    console.log(`设置 ${settingName} 已切换为:`, userConfig[settingName]);
                }
            });
        });
        ...
        if (saveBtn) {
            ...
            const newSaveBtn = settingsPanel.querySelector('#aa-settings-save');
        ...
        if (resetBtn) {
            ...
            const newResetBtn = settingsPanel.querySelector('#aa-settings-reset');
    ...
    function saveSettings() {
        ...
        try {
            ...
            const shadowColorInput = settingsPanel.querySelector('#aa-shadowColor');
            const shadowXInput = settingsPanel.querySelector('#aa-shadowX');
            const shadowYInput = settingsPanel.querySelector('#aa-shadowY');
            const shadowBlurInput = settingsPanel.querySelector('#aa-shadowBlur');
            ...
            if (shadowColorInput) {
                userConfig.shadowColor = hexToRgba(shadowColorInput.value || '#000000', 0.1);
            }
            if (shadowXInput) userConfig.shadowX = parseInt(shadowXInput.value) || DEFAULT_CONFIG.shadowX;
            if (shadowYInput) userConfig.shadowY = parseInt(shadowYInput.value) || DEFAULT_CONFIG.shadowY;
            if (shadowBlurInput) userConfig.shadowBlur = parseInt(shadowBlurInput.value) || DEFAULT_CONFIG.shadowBlur;
    ...
    function removeVipBoxes() {
        ...
        console.log(`找到 ${vipBoxes.length} 个VIP信息框`);

        vipBoxes.forEach(box => {
            console.log('删除VIP信息框:', box);
            box.style.display = 'none';
            box.remove();
        });

        // 额外检查可能的相关元素
        const possibleVipElements = document.querySelectorAll('[class*="vip"], [class*="Vip"], [class*="VIP"]');
        possibleVipElements.forEach(el => {
            if (el.textContent.includes('VIP') || el.textContent.includes('会员')) {
    ...
    // 主入口
    function init() {
        console.log('考试宝AI解析美化脚本已启动 V0.6');
